...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_order_food_form'])) {
        // Selected Menus
        if(isset($_POST['selected_menus']) && is_array($_POST['selected_menus'])) {
            ...
            $order_id = null;
            $stmtGetMaxOrderId = $con->prepare("SELECT MAX(order_id) AS max_order_id FROM in_order");
            $stmtGetMaxOrderId->execute();
            $max_order_id_row = $stmtGetMaxOrderId->fetch();
            if ($max_order_id_row && $max_order_id_row['max_order_id'] !== null) {
                $order_id = $max_order_id_row['max_order_id'] + 1;
            } else {
    ...
    <form method="post" id="order_food_form" action="order_food.php">
            ...
            <div>
                <?php
                    ...
                    foreach ($menu_categories as $category) {
                        echo "<div class='text_header'><span>{$category['category_name']}</span></div>";
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo "<div>{$row['menu_name']}</div>";
                            echo "<div class='item_select_part'>";
                            echo "<div class='menu_price_field'><span style='font-weight: bold;'>{$row['menu_price']}$</span></div>";
                            echo "<div class='select_item_bttn'>";
                            echo "<div class='btn-group-toggle' data-toggle='buttons'>";
                            echo "<label class='menu_label item_label btn btn-secondary'>";
                            echo "<input type='checkbox' name='selected_menus[]' value='{$row['menu_id']}' autocomplete='off'>Select";
                            echo "</label>";
                            echo "</div>";
                            echo "</div>";
                            echo "</div>";
        ...
        <div style="overflow:auto;padding: 30px;">
            <div style="float:right;">
                ...
                <button type="submit" id="nextBtn" class="next_prev_buttons">Next</button>
            </div>
        </div>
